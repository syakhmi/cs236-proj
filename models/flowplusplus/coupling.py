...
class Coupling(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, num_blocks, num_components, drop_prob,
                 use_attn=True, aux_channels=None, condition_embd_size=0):
        super(Coupling, self).__init__()
        self.nn = NN(in_channels, mid_channels, num_blocks, num_components, drop_prob, use_attn, aux_channels, condition_embd_size=condition_embd_size)

    def forward(self, x, sldj=None, reverse=False, aux=None, condition_embd=None):
        x_change, x_id = x
        a, b, pi, mu, s = self.nn(x_id, aux, condition_embd=condition_embd)
