...
def train(model, embedder, optimizer, scheduler,
    ...
    for epoch in range(opt.n_epochs):
        ...
        val_bpd = eval(model, embedder, val_loader, opt, writer, device)
...
def eval(model, embedder, test_loader, opt, writer, device):
...
def main(args=None):
    ...
    if opt.dataset == "imagenet32":
        train_dataset = Imagenet32Dataset(train=not opt.train_on_val, max_size=1 if opt.debug else opt.train_size)
        val_dataset = Imagenet32Dataset(train=0, max_size=1 if opt.debug else opt.val_size, start_idx=opt.val_start_idx)
    else:
