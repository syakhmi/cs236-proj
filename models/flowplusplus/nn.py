...
class NN(nn.Module):
    ...
    """
    def __init__(self, in_channels, num_channels, num_blocks, num_components, drop_prob, use_attn=True, aux_channels=None, condition_embd_size=0):
        ...
        if condition_embd_size > 0:
            self.conditioning_projection = nn.Linear(condition_embd_size, 2 * in_channels * (2 + 3 * self.k), bias=False)

    def forward(self, x, aux=None, condition_embd=None):
        b, c, h, w = x.size()
        x = self.in_conv(x)
        ...
        if condition_embd is not None:
            embedding_projected = self.conditioning_projection(condition_embd)
            x = torch.cat([x, x], dim=1)
            x = x + embedding_projected.view(x.size(0), -1, 1, 1)
            a_chunk, b_chunk = torch.chunk(x, 2, dim=1)
            x = a_chunk * torch.sigmoid(b_chunk)
